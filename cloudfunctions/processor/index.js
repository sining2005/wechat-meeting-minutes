...
async function tencentStep(config,job,segment){
  ...
  if(!job.externalTaskId){
    ...
    const common={EngineModelType:config.model||"16k_zh",ChannelNum:1,ResTextFormat:3,SpeakerDiarization:1,SpeakerNumber:0};
    let r;
  ...
  if(r.Data.Status===0||r.Data.Status===1)return {pending:true};if(r.Data.Status!==2)throw new Error(r.Data.ErrorMsg||"腾讯云语音识别失败");const details=Array.isArray(r.Data.ResultDetail)?r.Data.ResultDetail:[];const segments=details.map(x=>{const speakerId=x.SpeakerId!==null&&x.SpeakerId!==undefined&&Number.isInteger(Number(x.SpeakerId))?Number(x.SpeakerId):null;const speaker=speakerId===null?null:`说话人${speakerId+1}`;return {start:(Number(x.StartMs)||0)/1000,end:(Number(x.EndMs)||0)/1000,speakerId,speaker,text:String(x.FinalSentence||x.WrittenText||"").trim()};}).filter(x=>x.text);const text=segments.length?segments.map(x=>`${x.speaker||"说话人"}：${x.text}`).join("\n"):String(r.Data.Result||"");return {text,segments:segments.length?segments:[{start:0,end:segment.duration||0,speaker:null,speakerId:null,text}]};
}
